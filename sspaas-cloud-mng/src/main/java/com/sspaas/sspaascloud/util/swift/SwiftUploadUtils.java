...
public class SwiftUploadUtils {
	...
	public static String createObject(SwiftUploadPojo uploadPojo) {
		...
		System.out.println("swift文件上传工具类存储成功后返回文件路径：" + uploadPojo.getSwiftUrl() + os.getToken().getProject().getId()
				+ "/" + uploadPojo.getContainerName() + "/" + fileName + fileType);
		token = os.getToken(); 
	 ...
	 * @param uploadPojo
	 * @param uploadName swift存储文件名
	 * @param oldName	浏览器下载显示文件名
	 * @return
	...
	public static String getTempUrl(SwiftUploadPojo uploadPojo,String uploadName,String oldName) throws Exception {
		...
		String path =  uploadPojo.getTempUrlPref()+uploadPojo.getProjectId()+"/"+uploadPojo.getContainerName()+"/"+uploadName;
		...
		//拼接tempUrl路径
		return uploadPojo.getSwiftUrl()+uploadPojo.getProjectId()+"/"+uploadPojo.getContainerName()+"/"+uploadName
				+"?temp_url_sig="+tempUrlSig+"&temp_url_expires="+expires+"&filename="+oldName;
