...
<body class="gray-bg">
	<script type="text/javascript">
        $(document).ready(function(){
            ...
            $("#list4").jqGrid({
                url:'<%=basePath%>/feedbackType/selectAllList',
...
//                height:420,//高度，表格高度。可为数值、百分比或'auto'
                ...
                shrinkToFit:true,
                colModel:[
                    {label:"类型Id",name:'feedbackTypeId',index:'feedbackTypeId', width:'15%',align:'center',hidden: true},
                    {label:"名称",name:'name',index:'name', width:'15%',align:'center'},
                    {label:"描述",name:'descript',index:'descript', width:'35%',align:'center'},
           ...
           //修改文件类型窗口
            $("#btn_edit").bind("click",function(){
                ...
                $("#up_descript").val(model.descript); 
                $("#feedbackTypeId").val(model.feedbackTypeId); 
            ...
            $("#btn_delete").bind("click",function(){
            	...
            	$("#del").dialog({
                    ...
                    buttons:{
                        ...
                        "确定":function(){
                            $.ajax({
                                url:'<%=basePath%>/feedbackType/deleteType?feedbackTypeId='+model.feedbackTypeId,
        ...
        function toVaild(){
        	...
        	$.ajax({
                 url:'<%=basePath%>/feedbackType/addType?name=' + name
						+ '&descript=' + descript,
				type : "post",
				async : true, //同步异步请求
				cache : false, //是否使用缓存
      ...
      //修改反馈类型
        function up_toVaild(){
        	...
        	$.ajax({
                 url:'<%=basePath%>/feedbackType/updateType?name=' + name
						+ '&descript=' + descript+'&feedbackTypeId='+feedbackTypeId,
				type : "post",
				async : true, //同步异步请求
				cache : false, //是否使用缓存
				datatype : 'json',
	...
	<div class="ibox-title" id="user_button">
		<a data-toggle="modal" class="btn btn-primary" id="btn_add">添加</a> <a
			data-toggle="modal" class="btn btn-primary" id="btn_edit">编辑</a> <a
			data-toggle="modal" id="btn_delete" class="btn btn-primary">删除</a>
	</div>
